      ...
      <div className="flex flex-col gap-2 py-4 md:flex-row md:justify-between md:gap-12 xl:py-8">
        {prev && prev.path ? (
          <div className="flex flex-col gap-1">
            ...
            <Link href={`/${prev.path}`}>
              <HighlightUnderline data-umami-event="post-nav-prev">{prev.title}</HighlightUnderline>
            </Link>
          </div>
        ) : (
          <div />
        ...
        {next && next.path && (
          <div className="flex flex-col items-end gap-1 text-right">
            ...
            <Link href={`/${next.path}`}>
              <HighlightUnderline data-umami-event="post-nav-next">{next.title}</HighlightUnderline>
            </Link>
          </div>
        )}
      </div>
